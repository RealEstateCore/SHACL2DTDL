...
namespace SHACL2DTDL
{
    class Program
    {
        public class Options
        {
            ...
            [Option('s', "ontologySource", Required = false, HelpText = "An identifier for the ontology source; will be used to generate DTMI:s per the following design, where interfaceName is the local name of a translated NodeShape, and ontologyName is the last segment of the translated shape's namespace: <dtmi:{ontologySource}:{ontologyName}:{interfaceName};1>.")]
        ...
        private static void GenerateInterfaces()
        {
            ...
            foreach(NodeShape shape in _shapesGraph.NodeShapes().Where(nodeShape => nodeShape.Node.IsClass() && !IsIgnored(nodeShape.Node))) {
                ...
                // If it doesn't have superclasses, implement generic name property, externalIDs, and customTags
                    ...
                    ILiteralNode externalIdsDtdlName = dtdlModel.CreateLiteralNode("identifiers");
                    ...
                    ILiteralNode schemaMapKeyName = dtdlModel.CreateLiteralNode("namespace");
                    ...
                    ILiteralNode schemaMapValueName = dtdlModel.CreateLiteralNode("identifier");
                    ...
                    ILiteralNode externalIdsDisplayName = dtdlModel.CreateLiteralNode("Identifiers", "en");
                    ...
                    IBlankNode customProperties = dtdlModel.CreateBlankNode();
                    dtdlModel.Assert(new Triple(interfaceNode, dtdl_contents, customProperties));
                    dtdlModel.Assert(new Triple(customProperties, rdfType, dtdl_Property));
                    ILiteralNode customPropertiesDtdlName = dtdlModel.CreateLiteralNode("customProperties");
                    dtdlModel.Assert(new Triple(customProperties, dtdl_name, customPropertiesDtdlName));

                    // cpSchema is nested map (string->(string->string))
                    IBlankNode cpOuterMap = dtdlModel.CreateBlankNode();
                    dtdlModel.Assert(new Triple(cpOuterMap, rdfType, dtdl_Map));

                    // Outer map key (namespace)
                    IBlankNode cpOuterKey = dtdlModel.CreateBlankNode();
                    dtdlModel.Assert(new Triple(cpOuterMap, dtdl_mapKey, cpOuterKey));
                    ILiteralNode cpOuterKeyName = dtdlModel.CreateLiteralNode("namespace");
                    dtdlModel.Assert(new Triple(cpOuterKey, dtdl_name, cpOuterKeyName));
                    IUriNode cpOuterKeySchema = dtdlModel.CreateUriNode(DTDL._string);
                    dtdlModel.Assert(new Triple(cpOuterKey, dtdl_schema, cpOuterKeySchema));

                    // Outer map value declaration (that in turn wraps inner map)
                    IBlankNode cpOuterMapValue = dtdlModel.CreateBlankNode();
                    dtdlModel.Assert(new Triple(cpOuterMap, dtdl_mapValue, cpOuterMapValue));
                    ILiteralNode cpOuterMapValueName = dtdlModel.CreateLiteralNode("properties");
                    dtdlModel.Assert(new Triple(cpOuterMapValue, dtdl_name, cpOuterMapValueName));

                    // Inner map
                    IBlankNode cpInnerMap = dtdlModel.CreateBlankNode();
                    dtdlModel.Assert(new Triple(cpOuterMapValue, dtdl_schema, cpInnerMap));
                    dtdlModel.Assert(new Triple(cpInnerMap, rdfType, dtdl_Map));
                    // Inner map key (cpPropertyKey)
                    IBlankNode cpPropertyKey = dtdlModel.CreateBlankNode();
                    dtdlModel.Assert(new Triple(cpInnerMap, dtdl_mapKey, cpPropertyKey));
                    ILiteralNode cpPropertyKeyName = dtdlModel.CreateLiteralNode("propertyName");
                    dtdlModel.Assert(new Triple(cpPropertyKey, dtdl_name, cpPropertyKeyName));
                    IUriNode cpPropertyKeySchema = dtdlModel.CreateUriNode(DTDL._string);
                    dtdlModel.Assert(new Triple(cpPropertyKey, dtdl_schema, cpPropertyKeySchema));
                    // Inner map value (cpPropertyValue)
                    IBlankNode cpPropertyValue = dtdlModel.CreateBlankNode();
                    dtdlModel.Assert(new Triple(cpInnerMap, dtdl_mapValue, cpPropertyValue));
                    ILiteralNode cpPropertyValueName = dtdlModel.CreateLiteralNode("propertyValue");
                    dtdlModel.Assert(new Triple(cpPropertyValue, dtdl_name, cpPropertyValueName));
                    IUriNode cpPropertyValueSchema = dtdlModel.CreateUriNode(DTDL._string);
                    dtdlModel.Assert(new Triple(cpPropertyValue, dtdl_schema, cpPropertyValueSchema));

                    // Now our string->(string->string map is complete, graft it onto the customProperties)
                    dtdlModel.Assert(new Triple(customProperties, dtdl_schema, cpOuterMap));
                    // Display name of Custom Properties property.
                    ILiteralNode customPropertiesDisplayName = dtdlModel.CreateLiteralNode("Custom Properties", "en");
                    dtdlModel.Assert(new Triple(customProperties, dtdl_displayName, customPropertiesDisplayName));
                    // Custom properties are writeable
                    ILiteralNode customPropertiesTrue = dtdlModel.CreateLiteralNode("true", new Uri(XmlSpecsHelper.XmlSchemaDataTypeBoolean));
                    dtdlModel.Assert(new Triple(customProperties, dtdl_writable, customPropertiesTrue));
        ...
        private static Uri GetUnitForSemanticType(Uri semanticTypeUri)
        {
             Dictionary<Uri, Uri> defaultDtdlSemanticUnits = new Dictionary<Uri, Uri>
                {
                    { DTDL.Acceleration, DTDL.metrePerSecondSquared},
                    { DTDL.Angle, DTDL.degreeOfArc},
                    { DTDL.AngularAcceleration, DTDL.radianPerSecondSquared},
                    { DTDL.AngularVelocity, DTDL.revolutionPerMinute},
                    { DTDL.Area, DTDL.squareMetre},
                    { DTDL.Capacitance, DTDL.microfarad},
                    { DTDL.Current, DTDL.ampere},
                    { DTDL.DataRate, DTDL.mebibitPerSecond},
                    { DTDL.DataSize, DTDL.mebibyte},
                    { DTDL.Density, DTDL.gramPerCubicMetre},
                    { DTDL.Distance, DTDL.metre},
                    { DTDL.Energy, DTDL.kilowattHour},
                    { DTDL.ElectricCharge, DTDL.coulomb},
                    { DTDL.Force, DTDL.newton},
                    { DTDL.Frequency, DTDL.hertz},
                    { DTDL.Humidity, DTDL.gramPerCubicMetre},
                    { DTDL.Illuminance, DTDL.lux },
                    { DTDL.Inductance, DTDL.henry },
                    { DTDL.Length, DTDL.metre },
                    { DTDL.Luminance, DTDL.candelaPerSquareMetre },
                    { DTDL.LuminousFlux, DTDL.lumen },
                    { DTDL.LuminousIntensity, DTDL.candela },
                    { DTDL.MagneticFlux, DTDL.weber },
                    { DTDL.Mass, DTDL.kilogram },
                    { DTDL.MassFlowRate, DTDL.kilogramPerHour },
                    { DTDL.Power, DTDL.watt },
                    { DTDL.Pressure, DTDL.pascal },
                    { DTDL.RelativeHumidity, DTDL.percent },
                    { DTDL.Resistance, DTDL.ohm },
                    { DTDL.SoundPressure, DTDL.decibel },
                    { DTDL.Temperature, DTDL.degreeCelsius },
                    { DTDL.Thrust, DTDL.newton },
                    { DTDL.TimeSpan, DTDL.second },
                    { DTDL.Torque, DTDL.newtonMetre },
                    { DTDL.Velocity, DTDL.metrePerSecond },
                    { DTDL.Voltage, DTDL.volt },
                    { DTDL.Volume, DTDL.litre },
                    { DTDL.VolumeFlowRate, DTDL.litrePerSecond },
                };
        ...
        private static Uri? GetQkAsSemanticType(Uri quantityKind)
        {
            Dictionary<Uri, Uri> qkToDtdlTypeMappings = new Dictionary<Uri, Uri>
                {
                    { QUDT.QuantityKindNS.Acceleration, DTDL.Acceleration},
                    { QUDT.QuantityKindNS.Angle, DTDL.Angle },
                    { QUDT.QuantityKindNS.AngularAcceleration, DTDL.AngularAcceleration},
                    { QUDT.QuantityKindNS.AngularVelocity, DTDL.AngularVelocity},
                    { QUDT.QuantityKindNS.Area, DTDL.Area},
                    { QUDT.QuantityKindNS.Capacitance, DTDL.Capacitance},
                    { QUDT.QuantityKindNS.DataRate, DTDL.DataRate},
                    { QUDT.QuantityKindNS.Density, DTDL.Density},
                    { QUDT.QuantityKindNS.Distance, DTDL.Distance},
                    { QUDT.QuantityKindNS.ElectricCurrent, DTDL.Current},
                    { QUDT.QuantityKindNS.Energy, DTDL.Energy },
                    { QUDT.QuantityKindNS.InformationEntropy, DTDL.DataSize},
                    { QUDT.QuantityKindNS.ElectricCharge, DTDL.ElectricCharge},
                    { QUDT.QuantityKindNS.Force, DTDL.Force},
                    { QUDT.QuantityKindNS.Frequency, DTDL.Frequency},
                    { QUDT.QuantityKindNS.AbsoluteHumidity, DTDL.Humidity},
                    { QUDT.QuantityKindNS.Illuminance, DTDL.Illuminance },
                    { QUDT.QuantityKindNS.Inductance, DTDL.Inductance },
                    { QUDT.QuantityKindNS.Length, DTDL.Length },
                    { QUDT.QuantityKindNS.Luminance, DTDL.Luminance },
                    { QUDT.QuantityKindNS.LuminousFlux, DTDL.LuminousFlux },
                    { QUDT.QuantityKindNS.LuminousIntensity, DTDL.LuminousIntensity },
                    { QUDT.QuantityKindNS.MagneticFlux, DTDL.MagneticFlux },
                    ...
                    { QUDT.QuantityKindNS.RelativeHumidity, DTDL.RelativeHumidity },
                    { QUDT.QuantityKindNS.Resistance, DTDL.Resistance },
                    { QUDT.QuantityKindNS.SoundPressure, DTDL.SoundPressure },
                    { QUDT.QuantityKindNS.Temperature, DTDL.Temperature },
                    { QUDT.QuantityKindNS.Thrust, DTDL.Thrust },
                    { QUDT.QuantityKindNS.Time, DTDL.TimeSpan },
                    { QUDT.QuantityKindNS.Torque, DTDL.Torque },
